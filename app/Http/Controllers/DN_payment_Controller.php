...
class DN_payment_Controller extends Controller
{
    ...
    public function index_payment_japfa()
    {
        Session::flash('url', 'Master Central');
        $user = auth()->user();
        // auth()->user()->tokens()->delete();
        ...
        // $token = $user->createToken('auth_token')->plainTextToken;
        // session(['token' => $token]);
        return view('dn_transaction.payment.index_japfa', compact('user'));
    }
    ...
    public function index_faktur_pajak_japfa()
    {
        Session::flash('url', 'Master Central');
        $user = auth()->user();
        // auth()->user()->tokens()->delete();
        ...
        // $token = $user->createToken('auth_token')->plainTextToken;
        // session(['token' => $token]);
        return view('dn_transaction.faktur_pajak.index_japfa', compact('user'));
    }
    ...
    public function get_data_dn_payment_japfa(Request $request)
    {
    ...
    public function get_list_pajak_japfa(Request $request)
    {
        $query = "
            WITH PotonganTerbaru AS (
                SELECT *,
            ...
            SELECT
                ...
                value_tagihan_dn,
                ISNULL(pt.value_potongan, 0) AS value_potongan,
                'japfa' salesdntagih_client_code,
                kode_faktur_pajak,
                note_kwitansi,
                p.value_ppn,
                bukti_potong_pph_23,
                CAST(p.created_at AS DATE) AS tgl_kwitansi
            FROM
            tr_tagih_sales_DN_kwitansi_japfa p
            LEFT JOIN PotonganTerbaru pt ON pt.no_kwitansi = p.no_kwitansi AND pt.rn = 1
            WHERE p.no_kwitansi IS NOT NULL
            AND p.status != 1
        ";
    ...
    public function store_payment_japfa(Request $request)
    {
        DB::beginTransaction();
        // dd($request->all());
        try {
            ...
            $kode_tahun_bulan2 = $tahun . $bulan2;
            $payment_data = DB::connection('ms_sql_hgs')
                ->table('tr_acc_transaksi_sales_DN_payment_h_japfa')
                ->select('salesdnpay_code_h')
                ->whereMonth('rec_dateupdate', Carbon::now('Asia/Jakarta')->month)
                ->whereYear('rec_dateupdate', Carbon::now('Asia/Jakarta')->year)
                ->orderBy('rec_dateupdate', 'desc')
            ...
            $new_number_payment = $last_number_payment ? str_pad((int)$last_number_payment + 1, 4, '0', STR_PAD_LEFT) : '0001';
            $new_Code_payment = sprintf('SDP-JAPFA-%s%s-%s', $tahun, $bulan2, $new_number_payment);
            ...
            $new_Code_co_desc = sprintf('Payment JAPFA: %s', $new_Code_payment);
            $new_number_tmain = $last_number_tmain ? str_pad((int)$last_number_tmain + 1, 6, '0', STR_PAD_LEFT) : '000001';
            $new_Code_tmain = sprintf('TMC-%s-%s', $kode_tahun_bulan2, $new_number_tmain);
            $detailDataArray = [];
            foreach ($tampunganData as $index => $detail) {
                $detailDataArray[] = [
                    'salesdnpay_code_h' => $new_Code_payment,
                    'salesdnpay_code_d' => sprintf('%s-%04d', $new_Code_payment, $index + 1),
                    'salesdnpay_tagih_code_h' => $detail['no_kwitansi'],
                    'salesdnpay_value' => $detail['value_tagihan_dn'],
                ];
            }
            // dd($detailDataArray);
            // insert ke tabel coa dan trans main
            ...
            foreach ($chunks as $chunk) {
                DB::connection('ms_sql_hgs')->table('tr_acc_transaksi_sales_DN_payment_d_japfa')->insert($chunk);
            }
            DB::commit();
            return response()->json([
                'message' => 'Data berhasil dimasukkan',
    ...
    public function cetakPDF_payment(Request $request)
    {
        ...
        $formattedDate = $data[0]->salesdnpay_Date;
        // dd($query);
    ...
    public function cetakPDF_payment_japfa(Request $request)
    {
        $code = $request->get('code');
        $client_code = $request->get('client_code');
        $user = auth()->user()->username;
        ...
        $query = "
            ...
            FROM
                tr_acc_transaksi_coa c
                JOIN tr_acc_transaksi_sales_DN_payment_h_japfa p ON c.transcoa_head_code = p.salesdnpay_code_h
                join ms_acc_coa coa on coa_code = transcoa_coa_code
            WHERE
                salesdnpay_code_h = '$code'
            ORDER BY
    ...
    public function store_faktur_pajak_japfa(Request $request)
    {
        try {
            $no_kwitansi =  $request->input('no_kwitansi');
            // dd($no_kwitansi);
            ...
            if ($borongan == 0) {
                ...
                foreach ($dataToSave as $detail) {
                    ...
                    // dd('');
                    DB::connection('ms_sql_hgs')->table('tr_tagih_sales_DN_kwitansi_japfa')
                        ->where('no_kwitansi', $detail['no_kwitansi'])
                        ->update([
                            'kode_faktur_pajak' => $detail['no_faktur_pajak'],
                            'value_est_pph_4' => $detail['pajak_psl_23'],
    ...
    public function store_faktur_pajak_confirm_japfa(Request $request)
    {
        try {
            $no_kwitansi =  $request->input('no_kwitansi');
            $detail_client = 'test';
            ...
            $headerdata = DB::connection('ms_sql_hgs')->select("
            SELECT * FROM tr_tagih_sales_DN_kwitansi_japfa where no_kwitansi = '$no_kwitansi'
            ");
